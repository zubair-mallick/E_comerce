...
export const invalidateCache =async ({product,order,admin,userId,orderId,productId}:InvalidateCacheProp )=>{
if(product){
    const productKeys:string[] =["latest-product","category","adminProducts",]
    if(typeof productId === "string") productKeys.push(`singleProduct-${productId}`);
    console.log("here")
...
if(order){
    const orderKeys:string[] =["all-orders",`my-orders-${userId}`,`singleorder-${orderId}`]
